...
from preferences import Preference, remove_dominated, \
    StrictProductPreferenceDict, SmallerPreferred
from trajectory_game import World, SplineTransitionPath, \
    Trajectory, AllTrajectories, RuleEvaluationResult
...
def score_trajectories(all_traj: AllTrajectories) -> \
...
def remove_dominated_trajectories(result: Dict[Trajectory, Dict[str, RuleEvaluationResult]]) \
        -> Dict[Trajectory, Dict[str, RuleEvaluationResult]]:
    pref_list: List[str] = []
    ...
    pref_dict: Dict[str, Preference[D]] = \
        {p: SmallerPreferred() for p in pref_list}
