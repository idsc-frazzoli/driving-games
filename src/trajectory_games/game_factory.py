...
def get_trajectory_game_players() -> Mapping[PlayerName, TrajectoryGamePlayer]:
    ...
    vg = VehicleGeometry(m=D("200"), w=D("1"), l=D("1"), colour=(1, 0, 0))
    ...
    p1 = PlayerName("P1")
    p2 = PlayerName("P2")
    state1 = VehicleState(x=D("0"), y=D("8"), th=D(pi / 2), v=D("8"), st=D("0"), t=D("0"))
    ...
    vg2 = VehicleGeometry(m=D("200"), w=D("0.7"), l=D("1.5"), colour=(0, 0, 1))
    ret = {
...
def get_highway_world(players: Mapping[PlayerName, TrajectoryGamePlayer]) -> World:
    s: List[D] = [D(_-5) for _ in range(25)]
...
def get_trajectory_game() -> TrajectoryGame:
    ...
    world: World = get_highway_world(players=players)
    ps = PossibilitySet()

    game = TrajectoryGame(world=world, game_players=players, ps=ps, get_outcomes=evaluate_metrics, game_vis=TrajGameVisualization(world=world))
    return game
