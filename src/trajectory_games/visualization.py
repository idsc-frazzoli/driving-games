...
class TrajGameVisualization(GameVisualization[VehicleState, Trajectory, TrajectoryWorld]):
    ...
    def plot(
        ...
        deltay_plot = 50
        plot_limits = [
            [ego_position[0] - deltax_plot, ego_position[0] + deltax_plot],
        ...
        if player_actions and player_refs is not None:
            ...
            for pname, pactions in player_actions.items():
                if pname == PlayerName("Ego"):
                    ...
                    lanes_colour = "red"
                self.plot_actions(
                    axis=axis, actions=pactions, lanes=player_refs, colour=lanes_colour
                )  # todo: add reference lanes
    ...
    def plot_outcomes(
        ...
        for pname, p_pref in player_prefs.items():
            ...
            if n_players > 1:
                ...
                axis = ax
            plot_pref(
                axis=axis, pref_graph=p_graph, player_outcomes=player_outcomes, player_name=pname, title_info=title_info
            )
            ax_n = ax_n + 1
