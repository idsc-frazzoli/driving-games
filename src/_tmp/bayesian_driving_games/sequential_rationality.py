...
from _tmp.bayesian_driving_games.structures_solution import BayesianGameNode, BayesianSolvingContext
    ...
    if a == 1:  # probabilty monad
        for k, v in mixed_outcome.p.items():
            for k1, v1 in k.items():
                if (t2, t1) in k1:
                    if k1 in x:
                        x[k1] = x[k1] + list(v1.support())[0] * weight * v
                    else:
                        x[k1] = list(v1.support())[0] * weight * v
                elif (t1, t2) in k1:
                    if k1 in x:
                        x[k1] = x[k1] + list(v1.support())[0] * weight * v
                    else:
