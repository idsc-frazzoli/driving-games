...
class SplineCurve(Generic[X], Curve[X]):
    ...
    def __init__(self, s: List[X], z: List[X], order: int):
        if len(s) != len(z):
...
class SplineTransitionPath(Generic[X], TransitionPath[X]):
    ...
    def __init__(self, s: List[X], x: List[X], y: List[X], order: int):
        ...
        self.points = SampledSequence(timestamps=p_s, values=p_xy)
        d_x = self.x.derivative_at_s(p_s, order=1)
        d_y = self.y.derivative_at_s(p_s, order=1)
        d_xy = list(zip(d_x, d_y))
        self.deriv = SampledSequence(timestamps=p_s, values=d_xy)
    ...
    def heading_at_s(self, s: List[float]) -> List[X]:
        ...
        def calc_heading(x: X, y: X) -> float:
            return atan2(y, x)
...
class Trajectory:
    ...
    def __init__(self, traj: List[VehicleState]):
        times: List[float] = [t.t for t in traj]
