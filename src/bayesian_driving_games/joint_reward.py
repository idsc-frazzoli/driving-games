...
from .structures import VehicleActions, VehicleGeometry, BayesianVehicleState

__all__ = ["VehicleJointReward"]


class VehicleJointReward(JointRewardStructure[BayesianVehicleState, VehicleActions, Collision]):
    def __init__(
    ...
    # @lru_cache(None)
    def is_joint_final_state(self, xs: Mapping[PlayerName, BayesianVehicleState]) -> FrozenSet[PlayerName]:
        res = collision_check(xs, self.geometries)
        return frozenset(res)

    def joint_reward(self, xs: Mapping[PlayerName, BayesianVehicleState]) -> Mapping[PlayerName, Collision]:
        res = collision_check(xs, self.geometries)
        return res


class IndividualJointReward(JointRewardStructure[BayesianVehicleState, VehicleActions, Collision]):
    def __init__(
            self, collision_threshold: float, geometries: Mapping[PlayerName, VehicleGeometry],
