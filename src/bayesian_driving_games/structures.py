...
from games import Dynamics, GamePlayer, Game, PlayerName, SolvingContext
from possibilities import Poss
from zuper_commons.types import ZValueError
from driving_games.rectangle import Rectangle

__all__ = ["PlayerType", "BayesianGamePlayer", "BayesianGame", "BayesianVehicleState",
           "BayesianVehicleDynamics", "BayesianSolvingContext"]
...
""" relaxed player type optimizes for comfort """
...
class BayesianVehicleState(VehicleState):
    ...
    def compare_physical_states(self, s2: "BayesianVehicleState") -> bool:
        ...
        if (self.ref != s2.ref or self.x != s2.x or self.v != s2.v or self.wait != s2.wait or
                self.light != s2.light or self.player_type == s2.player_type):
            # fixme check on player_type?
            return False
        else:
            return True
