...
def get_security_policies(
    ...
    actions: Dict[PlayerName, FrozenSet[U]] = {}
    for player_name in moves:
        player_pref = preferences[player_name]
        actions[player_name] = get_security_policy(ps, solved, moves, player_name, player_pref)
...
def get_security_policy(
    ...
    action2outcomes: Dict[U, FrozenSet[Outcome]] = {}
    for player_action in moves[player_name]:
        option_outcomes = what_if_player_chooses(ps, solved, player_name, player_action, others_choices)
...
def what_if_player_chooses(
    ...
    player_name: PlayerName,
    player_action: U,
    others_choices: Mapping[PlayerName, FrozenSet[U]],
) -> SetOfOutcomes:
    ...
    set_pure_actions_with_player = add_action(ps, player_name, player_action, set_pure_actions_others)
