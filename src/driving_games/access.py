...
from . import logger
from .game_def import (Dynamics, Game, GamePlayer, GamePlayerPreprocessed, GamePreprocessed,
...
def preprocess_game(game: Game, dt: D) -> GamePreprocessed:
    ...
    players_pre = {player_name: GamePlayerPreprocessed(get_player_graph(player, dt))
                   for player_name, player in game.players.items()}

    return GamePreprocessed(game=game, dt=dt, players_pre=players_pre,
                            game_graph=game_graph)


def get_accessible_states(initial: AbstractSet, personal_reward_structure: PersonalRewardStructure, dynamics:
Dynamics, dt: D) -> DiGraph:
    G = DiGraph()
...
def get_game_graph(game: Game, dt: D) -> DiGraph:
            ...
            for s1, s2 in product(s1s, s2s):
                ...
                if S2 not in G.nodes:
                    ...
                    G.add_node(S2, is_final2=is_final2, is_final1=is_final1, is_joint_final=is_joint_final,
                               is_initial=False, generation=generation + 1, in_game=in_game)
...
def compute_graph_layout(G: DiGraph, iterations=300) -> None:
...
def get_player_graph(player: GamePlayer, dt: D) -> DiGraph:
