...
from duckietown_world import SE2Transform, relative_pose, Matrix2D
...
class DgLanelet:
    ...
    def from_commonroad_lanelet(cls, lanelet: Lanelet) -> "DgLanelet":
        return cls.from_vertices(left_vertices=lanelet.left_vertices,
                                 right_vertices=lanelet.right_vertices,
                                 center_vertices=lanelet.center_vertices)

    @classmethod
    def from_vertices(cls, left_vertices: np.ndarray, right_vertices: np.ndarray,
                      center_vertices: np.ndarray) -> "DgLanelet":
        ctr_points = []
    ...
    @classmethod
    def from_start(cls, lane_network: LaneletNetwork, start: np.ndarray,
                   max_length: float = 50.0) -> List["DgLanelet"]:
        init_lane: Lanelet = cls.get_lanelets(lane_network=lane_network, points=[start])[0]
        lanes_all, _ = Lanelet.all_lanelets_by_merging_successors_from_lanelet(lanelet=init_lane,
                                                                               network=lane_network,
                                                                               max_length=max_length)
        return [cls.from_commonroad_lanelet(lane) for lane in lanes_all]

    @classmethod
    def from_ends(cls, lane_network: LaneletNetwork, start: np.ndarray,
                  goals: Sequence[np.ndarray], n_interp: int = 5) -> List["DgLanelet"]:
        init_lane: Lanelet = cls.get_lanelets(lane_network=lane_network, points=[start])[0]
        ...
        q0_l, q0_c, q0_r = get_qmatrices(cv=init_lane.center_vertices[-1],
                                         lv=init_lane.left_vertices[-1],
                                         rv=init_lane.right_vertices[-1])
        all_lanes: List[DgLanelet] = []
        for goal_lane in goal_lanes:
            q1_l, q1_c, q1_r = get_qmatrices(cv=goal_lane.center_vertices[0],
                                             lv=goal_lane.left_vertices[0],
                                             rv=goal_lane.right_vertices[0])
            ...
            all_lanes.append(cls.from_vertices(left_vertices=left, right_vertices=right,
