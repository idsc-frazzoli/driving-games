...
from duckie_games.structures import DuckieCosts


class DuckiePreferencesCollTime(Preference[Combined[Collision, DuckieCosts]]):
    ...
    def get_type(self) -> Type[Combined[Collision, D]]:
        return Combined[Collision, DuckieCosts]

    def __repr__(self) -> str:
        d = {"P": self.get_type(), "lexi": self.lexi}
        return "DuckiePreferencesCollTime: " + debug_print(d)

    def compare(
        self, a: Combined[Collision, DuckieCosts], b: Combined[Collision, DuckieCosts]
    ) -> ComparisonOutcome:
