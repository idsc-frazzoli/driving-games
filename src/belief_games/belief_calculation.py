...
def belief_calc_simple1(p0: TwoVehicleSimpleParams):
    ...
    p = (D(1)/p0.side)*p0.first_progress
    q = (D(1)/p0.side)*p0.second_progress
    ...
    belief["Game_SM"] = (D(1)-p)*(D(1)-q)*D(0.4)
    belief["Game_Normal"] = (D(1)-p)*(D(1)-q)*D(0.6)
...
def belief_calc_simple2(p0: TwoVehicleSimpleParams):
    ...
    belief = {}
    p = (D(1)/p0.side)*p0.second_progress
    q = (D(1)/p0.side)*p0.first_progress
