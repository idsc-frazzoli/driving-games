...
__all__ = ["MpcFullKinCont", "MpccFullKinCont"]
...
class MpcFullKinCont(MpcKinBase):
    ...
    def set_bounds(self):
        ...
        self.mpc.set_nl_cons('lb_left',
                             ...
                             ub=0)
        self.mpc.set_nl_cons('ub_left', (1 - self.homotopy) * (state_d - self.constraints_obs(state_s, self.obstacle_state)[0][1] + 1.5), ub=0)
...
class MpccFullKinCont(MpccKinBase):
    ...
    def set_up_mpc(self):
        ...
        lterm = self.lterm(speed_ref=10)
        mterm = self.mterm(speed_ref=10)
