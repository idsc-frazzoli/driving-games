...
def visualize_traj(
    traj: DgSampledSequence[SE2value], player: PlayerName, ax: Axes, color="b", plot_occupancy=True, is_ref=False
):
...
def visualize_car(pose: SE2value, ax: Axes, color="b", is_ref=False):
    ...
    outline = np.array([[-l_r, l_f, l_f, -l_r, -l_r], [w_half, w_half, -w_half, -w_half, w_half]])
    ...
    if is_ref:
        ...
        alpha = 1
    car = patches.Polygon(gk[0:2, :].T, color=color, zorder=51, alpha=alpha)
...
def visualize_pose(pose: SE2value, ax: Axes):
    ...
    dx = 3 * np.cos(theta)
    dy = 3 * np.sin(theta)
    ax.arrow(x=t[0], y=t[1], dx=dx, dy=dy, width=0.2, color="r", zorder=52)
...
def visualize_box_2d(
    ...
    rect_init = patches.Rectangle(
        (center[0] - w_s12_init / 2, center[1] - w_s21_init / 2), w_s12_init, w_s21_init, linewidth=1, edgecolor="r"
    )
...
def visualize_box_3d(
    ...
    cube1 = (
        (center12[0] - w_s12 / 2 < x)
        & (x < center12[0] + w_s12 / 2)
        & (center12[1] - w_s21 / 2 < y)
        & (y < center12[1] + w_s21 / 2)
        & (z < s_max[player3])
    )
    cube2 = (
        (center13[0] - w_s13 / 2 < x)
        & (x < center13[0] + w_s13 / 2)
        & (center13[1] - w_s31 / 2 < z)
        & (z < center13[1] + w_s31 / 2)
        & (y < s_max[player2])
    )
    cube3 = (
        (center23[0] - w_s23 / 2 < y)
        & (y < center23[0] + w_s23 / 2)
        & (center23[1] - w_s32 / 2 < z)
        & (z < center23[1] + w_s32 / 2)
        & (x < s_max[player1])
    )
