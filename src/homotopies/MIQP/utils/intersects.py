...
def find_intersects(trajs: Dict[PlayerName, DgSampledSequence[SE2value]]) -> Dict[PlayerName, Dict[PlayerName, float]]:
    ...
    for player_pair in combinations(trajs.keys(), 2):
        ...
        if (
            len(intersects_w) != 0
        ):  # todo: currently only record the first intersection if 2 paths intersect more than once
...
def check_intersect(
    ...
    buffer = 0.0001
    if (
        max(line1[:, 0]) + buffer >= x >= min(line1[:, 0]) - buffer
...
def find_p_idx(path: List[Tuple[float, float]], point: Tuple[float, float]) -> Tuple[int, float]:
    ...
    if min_idx == length - 1:
        ...
        p1_idx = min_idx
        p2_idx = min_idx + 1
    elif max(path[min_idx : min_idx + 2, 0]) >= point[0] >= min(path[min_idx : min_idx + 2, 0]) and max(
        path[min_idx : min_idx + 2, 1]
    ) >= point[1] >= min(path[min_idx : min_idx + 2, 0]):
