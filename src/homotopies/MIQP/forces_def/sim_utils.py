...
__all__ = ["get_init_bin", "update_sim", "extract_plans", "check_status", "update_evaluation"]
...
def update_sim(X, ddS, n_controlled):
    ...
    for p_idx in range(n_controlled):
        ...
        next_X[x_idx] = np.dot(params.C, curr).reshape(
            [
                2,
            ]
        )
...
def extract_plans(solverout, n_controlled, n_inter):
    ...
    for stage_idx in range(params.N):
        for p_idx in range(n_controlled):
            X_plan[params.n_states * p_idx : params.n_states * (p_idx + 1), stage_idx] = solverout[
                "x{stage:02d}{player:01d}".format(stage=stage_idx + 1, player=p_idx + 1)
            ]
            dds_plan[params.n_cinputs * p_idx : params.n_cinputs * (p_idx + 1), stage_idx] = solverout[
                "u{stage:02d}{player:01d}".format(stage=stage_idx + 1, player=p_idx + 1)
            ]
        for i_idx in range(n_inter):
            bin_plan[params.n_binputs * i_idx : params.n_binputs * (i_idx + 1), stage_idx] = solverout[
