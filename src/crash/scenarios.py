...
def get_scenario_01() -> SimContext:
    ...
    x0_p4 = VehicleState(x=2, y=15, theta=-0.1, vx=5, delta=0)
    x0_p5 = VehicleState(x=27, y=4, theta=deg2rad(95), vx=6, delta=0)
    x0_p6 = VehicleState(x=17, y=35, theta=deg2rad(-90), vx=4, delta=0)
    models = {P1: VehicleModel.default_car(x0_p1),
              ...
              P5: VehicleModel.default_bicycle(x0_p5),
              P6: VehicleModel.default_car(x0_p6)}
    ...
    return SimContext(map_name="4way-double-intersection-only",
                      ...
                      players=players,
                      log=SimulationLog(),
                      param=SimParameters(dt=D(0.05), sim_time_after_collision=D(5)),
                      )
